...
def llmGenerate(lang : str, code_path : str):
    ...
    file_path = ""  
    # Replace with the desired path and filename for multiple languages
    if lang == "PYTHON":
        file_path = f"{code_path.split('/')[-1][:-4]}.py"
    elif lang == "R":
        file_path = f"{code_path.split('/')[-1][:-4]}.R"
    elif lang == "JAVASCRIPT":
        file_path = f"{code_path.split('/')[-1][:-4]}.js"
    elif lang == "JULIA":
        file_path = f"{code_path.split('/')[-1][:-4]}.jl"
    elif lang == "JAVA":
        file_path = f"{code_path.split('/')[-1][:-4]}.java"
    elif lang == "C":
        file_path = f"{code_path.split('/')[-1][:-4]}.c"
    elif lang == "C++":
        file_path = f"{code_path.split('/')[-1][:-4]}.cpp"
    elif lang == "GO":
        file_path = f"{code_path.split('/')[-1][:-4]}.go"
    elif lang == "RUST":
        file_path = f"{code_path.split('/')[-1][:-4]}.rs"
    elif lang == "RUBY":
        file_path = f"{code_path.split('/')[-1][:-4]}.rb"
